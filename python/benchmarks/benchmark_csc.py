...
def build_scipy_csc(
    ...
    A_csc = A_coo.tocsc()
    return A_csc, int(A_coo.nnz)


def build_scipy_csc_pair(
    m: int, n: int, density: float, seed: int, dtype: np.dtype
) -> Tuple[sp.csc_matrix, sp.csc_matrix, int, int]:
    A, nnzA = build_scipy_csc(m, n, density, seed, dtype)
    B, nnzB = build_scipy_csc(m, n, density, seed + 101, dtype)
...
def build_lacuna_csc_from_scipy(A_scipy: sp.csc_matrix):
    try:
        from lacuna.sparse.csc import CSC
    except Exception:
        return None
    csc = A_scipy.tocsc()
    data64 = csc.data.astype(np.float64, copy=False)
    try:
        return CSC(csc.indptr, csc.indices, data64, csc.shape, check=False)
    except Exception:
        return None
...
def run_spmv_scipy(A: sp.csc_matrix, x: np.ndarray) -> np.ndarray:
...
def run_spmm_scipy(A: sp.csc_matrix, B: np.ndarray) -> np.ndarray:
...
def run_sum_scipy(A: sp.csc_matrix) -> float:
    return float(A.sum())


def run_sum_axis_scipy(A: sp.csc_matrix, axis: int) -> np.ndarray:
...
def run_prune_scipy(A: sp.csc_matrix, eps: float) -> sp.csc_matrix:
    B = A.copy().tocsc()
    if B.data.size:
...
def run_eliminate_scipy(A: sp.csc_matrix) -> sp.csc_matrix:
    B = A.copy().tocsc()
    B.eliminate_zeros()
    return B
...
def run_add_scipy(A: sp.csc_matrix, B: sp.csc_matrix) -> sp.csc_matrix:
    return (A + B).tocsc()
...
def run_mul_scipy(A: sp.csc_matrix, alpha: float) -> sp.csc_matrix:
    return (alpha * A).tocsc()
...
def run_sub_scipy(A: sp.csc_matrix, B: sp.csc_matrix) -> sp.csc_matrix:
    return (A - B).tocsc()
...
def run_hadamard_scipy(A: sp.csc_matrix, B: sp.csc_matrix) -> sp.csc_matrix:
    return A.multiply(B).tocsc()
...
def main():
    p = argparse.ArgumentParser(description="CSC benchmarks across backends for multiple kernels")
    ...
    p.add_argument(
        ...
        help=(
            "Comma-separated ops: "
            "spmv, spmm, sum, row_sums, col_sums, prune, eliminate, add, sub, mul, hadamard"
        ),
    )
    ...
    A_scipy, nnz = build_scipy_csc(args.m, args.n, args.density, args.seed, dtype)
    A_scipy2, _ = build_scipy_csc(args.m, args.n, args.density, args.seed + 7, dtype)
    ...
    A_lacuna = None if args.no_lacuna else build_lacuna_csc_from_scipy(A_scipy)
    B_lacuna = None if args.no_lacuna else build_lacuna_csc_from_scipy(A_scipy2)
    ...
    # ---- print summary ----
    print(
        f"CSC Benchmarks: m={args.m} n={args.n} k={args.k} density={args.density} dtype={args.dtype} nnz={nnz}"
    )
    for r in results:
        if not r:
            continue
