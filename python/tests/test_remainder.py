...
def test_csr_scalar_remainder():
    A = make_csr()
    R = A % 2.0
    np.testing.assert_allclose(R.toarray(), A.toarray() % 2.0)


def test_csr_pair_remainder_zero():
    A = make_csr()
    Z = A.remainder(A)
    assert Z.nnz == 0
...
def test_csc_scalar_remainder():
    A = make_csc()
    R = A % 2.0
    np.testing.assert_allclose(R.toarray(), A.toarray() % 2.0)


def test_csc_pair_remainder_basic():
    A = make_csc()
    # B: same structure, values [2,2,5]
    B = CSC(
        np.array([0, 1, 2, 3], dtype=np.int64),
        np.array([0, 1, 0], dtype=np.int64),
        np.array([2.0, 2.0, 5.0], dtype=np.float64),
        (2, 3),
        check=False,
    )
    C = A.remainder(B)
    # Sparse semantics: only positions where both A and B are nonzero are computed
    Ad = A.toarray()
    Bd = B.toarray()
    mask = (Ad != 0.0) & (Bd != 0.0)
    exp = np.zeros_like(Ad)
    exp[mask] = np.remainder(Ad[mask], Bd[mask])
    np.testing.assert_allclose(C.toarray(), exp)


def test_coo_scalar_remainder():
    A = make_coo()
    R = A % 2.0
    np.testing.assert_allclose(R.toarray(), A.toarray() % 2.0)


def test_coond_scalar_remainder():
    shape = (2, 3, 2)
...
def test_array_api_remainder_scalar_and_pair():
    A = make_csr()
    B = CSR(A.indptr, A.indices, np.array([2.0, 5.0, 2.0]), A.shape, check=False)
    R1 = xp.remainder(A, 2.0)
    R2 = xp.remainder(A, B)
    np.testing.assert_allclose(R1.toarray(), (A % 2.0).toarray())
    np.testing.assert_allclose(R2.toarray(), A.remainder(B).toarray())


def test_array_api_remainder_errors():
    A = make_csr()
    with pytest.raises(NotImplementedError):
        _ = xp.remainder(2.0, A)


def test_namespace_capability_has_remainder():
    info = xp.__array_namespace_info__()
    caps = info["capabilities"]
    assert "remainder" in caps.get("elementwise", [])
