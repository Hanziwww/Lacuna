...
def generate_svg(sections, out_path: Path):
    ...
    svg.append(
        f"<rect class='card' x='0' y='0' width='{width}' height='{height}' rx='10' ry='10' />"
    )
    svg.append(f"<text class='title' x='{padding}' y='{padding + 24}'>Array API Roadmap</text>")
    svg.append(
        f"<text class='meta' x='{padding}' y='{padding + 48}'>Source: docs/TODO.md · Updated by CI</text>"
    )
    ...
    for i, s in enumerate(sections, start=0):
        ...
        svg.append(
            f"<text class='label' x='{padding}' y='{y + row_h / 2}' dominant-baseline='middle'>{label}</text>"
        )
        ...
        by = y
        svg.append(
            f"<rect class='bar-bg' x='{bx}' y='{by}' width='{bar_w}' height='{row_h}' rx='10' ry='10' />"
        )
        fill_w = int(bar_w * pct / 100)
        fill_class = (
            "bar-fill-low" if pct < 34 else ("bar-fill-mid" if pct < 67 else "bar-fill-high")
        )
        if fill_w > 0:
            svg.append(
                f"<rect class='{fill_class}' x='{bx}' y='{by}' width='{fill_w}' height='{row_h}' rx='10' ry='10' filter='url(#shadow)' />"
            )
        ...
        svg.append(
            f"<text class='{pct_class}' x='{bx + bar_w - 8}' y='{by + row_h / 2}' dominant-baseline='middle' text-anchor='end'>{escape(pct_text)}</text>"
        )
    ...
    svg.append(
        f"<text class='sub' x='{padding}' y='{y + row_h / 2}' dominant-baseline='middle'>Overall</text>"
    )
    bx = padding + label_w
    by = y
    svg.append(
    ...
    fill_class = (
        "bar-fill-low" if ov_pct < 34 else ("bar-fill-mid" if ov_pct < 67 else "bar-fill-high")
    )
    if fill_w > 0:
        svg.append(
            f"<rect class='{fill_class}' x='{bx}' y='{by}' width='{fill_w}' height='{row_h}' rx='10' ry='10' filter='url(#shadow)' />"
    ...
    svg.append(
        f"<text class='{ov_pct_class}' x='{bx + bar_w - 8}' y='{by + row_h / 2}' dominant-baseline='middle' text-anchor='end'>{int(round(ov_pct))}% ({ov_c}/{ov_t})</text>"
    )
