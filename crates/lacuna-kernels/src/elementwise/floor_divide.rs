//! Element-wise floor division kernels for sparse structures.
//!
//! Implements:
//! - floor_divide for CSR and CSC (sparse ⊘ sparse, intersection-of-support)
//! - floor_divide by scalar for CSR/CSC/COO/COOND (per-element floor(data/alpha))
//!
//! Parallelized across rows/columns for CSR/CSC two-pass algorithms and
//! chunk-parallel over data arrays for scalar variants. Vectorization for floor
//! is not used due to lack of portable floor on wide vectors; however, the
//! division itself benefits from CPU pipelines and parallelism.
...
/// Count non-zero entries for floor division of two sorted rows (or columns).
/// Only positions where both have entries are considered; duplicates in either
/// input are coalesced by summation before division.
#[inline]
unsafe fn floordiv_row_count(
    ai: *const i64,
    av: *const f64,
    alen: usize,
    bi: *const i64,
...
/// Fill floor-division results of two rows (or columns) into output buffers.
#[inline]
#[allow(clippy::too_many_arguments)]
unsafe fn floordiv_row_fill(
    ai: *const i64,
    av: *const f64,
    alen: usize,
    bi: *const i64,
...
#[must_use]
pub fn floordiv_csr_f64_i64(a: &Csr<f64, i64>, b: &Csr<f64, i64>) -> Csr<f64, i64> {
...
#[must_use]
pub fn floordiv_csc_f64_i64(a: &Csc<f64, i64>, b: &Csc<f64, i64>) -> Csc<f64, i64> {
    assert_eq!(a.nrows, b.nrows);
    assert_eq!(a.ncols, b.ncols);
    let ncols = a.ncols;
...
/// Floor divide by scalar for CSR: B = floor(A / alpha)
#[must_use]
pub fn floordiv_scalar_f64(a: &Csr<f64, i64>, alpha: f64) -> Csr<f64, i64> {
    // Caller should guard alpha != 0.0; do not special-case here.
...
/// Floor divide by scalar for CSC
#[must_use]
pub fn floordiv_scalar_csc_f64(a: &Csc<f64, i64>, alpha: f64) -> Csc<f64, i64> {
    let mut data = a.data.clone();
    let chunk = 4096usize;
    data.par_chunks_mut(chunk).for_each(|chunk| {
        for v in chunk {
...
/// Floor divide by scalar for COO
#[must_use]
pub fn floordiv_scalar_coo_f64(a: &Coo<f64, i64>, alpha: f64) -> Coo<f64, i64> {
    let mut data = a.data.clone();
    let chunk = 4096usize;
    data.par_chunks_mut(chunk).for_each(|chunk| {
        for v in chunk {
...
/// Floor divide by scalar for COOND
#[must_use]
pub fn floordiv_scalar_coond_f64(a: &CooNd<f64, i64>, alpha: f64) -> CooNd<f64, i64> {
    let mut data = a.data.clone();
    let chunk = 4096usize;
    data.par_chunks_mut(chunk).for_each(|chunk| {
        for v in chunk {
