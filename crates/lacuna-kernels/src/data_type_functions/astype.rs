...
use crate::util::{i64_to_usize, SMALL_NNZ_LIMIT};
...
pub fn csr_to_csc_f64_i64(a: &Csr<f64, i64>) -> Csc<f64, i64> {
    ...
    for &j in &a.indices { indptr[i64_to_usize(j) + 1] += 1; }
    for j in 0..ncols { indptr[j + 1] += indptr[j]; }
...
pub fn csc_to_csr_f64_i64(a: &Csc<f64, i64>) -> Csr<f64, i64> {
    ...
    for &i in &a.indices { indptr[i64_to_usize(i) + 1] += 1; }
    for r in 0..nrows { indptr[r + 1] += indptr[r]; }
    let mut indices = vec![0i64; nnz];
    let mut data = vec![0.0f64; nnz];
    let next: Vec<AtomicI64> = indptr.iter().copied().map(AtomicI64::new).collect();
    let indices_addr = indices.as_mut_ptr() as usize;
...
pub fn coo_to_csr_f64_i64(a: &Coo<f64, i64>) -> Csr<f64, i64> {
    ...
    for (r, c, v) in triples {
        if r != cur_row {
            ...
            for ptr in indptr.iter_mut().take(r_us + 1).skip(prev_us) { *ptr = len_i64; }
            cur_row = r;
            last_col = c;
            acc = v;
            continue;
        }
        if c == last_col { acc += v; } else {
            indices.push(last_col);
            data.push(acc);
            last_col = c;
            acc = v;
    ...
    if cur_row >= 0 && last_col >= 0 { indices.push(last_col); data.push(acc); }
    let start_row = if cur_row < 0 { 0 } else { i64_to_usize(cur_row + 1) };
    let len_i64 = i64::try_from(indices.len()).unwrap_or(i64::MAX);
    for ptr in indptr.iter_mut().take(nrows + 1).skip(start_row) { *ptr = len_i64; }
    Csr::from_parts_unchecked(nrows, ncols, indptr, indices, data)
}
...
pub fn coo_to_csc_f64_i64(a: &Coo<f64, i64>) -> Csc<f64, i64> {
    let coo_t = Coo::from_parts_unchecked(
        a.ncols, a.nrows, a.col.clone(), a.row.clone(), a.data.clone(),
    );
...
fn product_checked(dims: &[usize]) -> usize {
    let mut acc: usize = 1;
    for &x in dims { acc = acc.checked_mul(x).expect("shape product overflow"); }
    acc
}
...
fn build_strides_row_major(dims: &[usize]) -> Vec<usize> {
    ...
    strides[n - 1] = 1;
    for i in (0..n - 1).rev() { strides[i] = strides[i + 1].checked_mul(dims[i + 1]).expect("shape product overflow"); }
    strides
}
...
fn coond_axes_to_coo_f64_i64(a: &CooNd<f64, i64>, row_axes: &[usize]) -> Coo<f64, i64> {
    ...
    for &ax in row_axes { assert!(ax < ndim, "row axis out of bounds"); assert!(!used[ax], "duplicate axis in row_axes"); used[ax] = true; }
    let mut col_axes: Vec<usize> = Vec::with_capacity(ndim - row_axes.len());
    for (d, &flag) in used.iter().enumerate().take(ndim) { if !flag { col_axes.push(d); } }
    ...
    let nnz = a.data.len();
    if nnz == 0 { return Coo::from_parts_unchecked(nrows, ncols, Vec::new(), Vec::new(), Vec::new()); }
    let mut row = vec![0i64; nnz];
    let mut col = vec![0i64; nnz];
    if nnz < SMALL_NNZ_LIMIT {
        for k in 0..nnz {
            ...
            for (m, &d) in row_axes.iter().enumerate() {
                ...
                r = r.checked_add(idx.checked_mul(s).expect("linear index overflow")).expect("linear index overflow");
            }
            let mut c2: usize = 0;
            for (m, &d) in col_axes.iter().enumerate() {
                let idx = i64_to_usize(unsafe { *a.indices.get_unchecked(base + d) });
                let s = if col_strides.is_empty() { 0 } else { col_strides[m] };
                c2 = c2.checked_add(idx.checked_mul(s).expect("linear index overflow")).expect("linear index overflow");
            }
