...
pub fn prune_eps_coond(a: &CooNd<f64, i64>, eps: f64) -> CooNd<f64, i64> {
    if eps < 0.0 { return a.clone(); }
    let nnz = a.data.len();
    if nnz == 0 { return a.clone(); }
    ...
    for k in 0..nnz {
        ...
        if v.abs() > eps {
            data.push(v);
            let base = k * ndim;
            for d in 0..ndim { indices.push(a.indices[base + d]); }
        }
    }
...
pub fn prune_eps_csc(a: &Csc<f64, i64>, eps: f64) -> Csc<f64, i64> {
    ...
    if nnz_total < SMALL_NNZ_PRUNE {
        ...
        for j in 0..ncols {
            ...
            while k < limit4 {
                let v = unsafe { let p = col.as_ptr().add(k).cast::<[f64; 4]>(); f64x4::new(core::ptr::read_unaligned(p)) };
                let arr = v.to_array();
                if arr[0].abs() > eps { cnt += 1; }
                if arr[1].abs() > eps { cnt += 1; }
                if arr[2].abs() > eps { cnt += 1; }
                if arr[3].abs() > eps { cnt += 1; }
                k += 4;
            }
            while k < col.len() { if col[k].abs() > eps { cnt += 1; } k += 1; }
        ...
        for j in 0..ncols {
            ...
            for p in s..e {
                let v = a.data[p];
                if v.abs() > eps { indices[dst] = a.indices[p]; data[dst] = v; dst += 1; }
            }
        }
    ...
    let mut indptr = vec![0i64; ncols + 1];
    for j in 0..ncols { indptr[j + 1] = indptr[j] + usize_to_i64(counts[j]); }
    let nnz = i64_to_usize(indptr[ncols]);
    let mut indices = vec![0i64; nnz];
    let mut data = vec![0.0f64; nnz];
    ...
    (0..ncols).into_par_iter().for_each(move |j| {
        ...
        unsafe {
            let pi = (pi_addr as *mut i64).add(0);
            let pv = (pv_addr as *mut f64).add(0);
            for p in s..e {
                let v = a.data[p];
                if v.abs() > eps {
...
pub fn prune_eps_coo(a: &Coo<f64, i64>, eps: f64) -> Coo<f64, i64> {
    ...
    for k in 0..a.data.len() {
        let v = a.data[k];
        if v.abs() > eps { row.push(a.row[k]); col.push(a.col[k]); data.push(v); }
    }
...
pub fn prune_eps(a: &Csr<f64, i64>, eps: f64) -> Csr<f64, i64> {
    ...
    if nnz_total < SMALL_NNZ_PRUNE {
        ...
        for i in 0..nrows {
            ...
            while k < limit4 {
                let v = unsafe { let p = row.as_ptr().add(k).cast::<[f64; 4]>(); f64x4::new(core::ptr::read_unaligned(p)) };
                let arr = v.to_array();
                if arr[0].abs() > eps { cnt += 1; }
                if arr[1].abs() > eps { cnt += 1; }
                if arr[2].abs() > eps { cnt += 1; }
            ...
            while k < row.len() { if row[k].abs() > eps { cnt += 1; } k += 1; }
    ...
    let mut indptr = vec![0i64; nrows + 1];
    for i in 0..nrows { indptr[i + 1] = indptr[i] + usize_to_i64(counts[i]); }
    let nnz = i64_to_usize(indptr[nrows]);
    let mut indices = vec![0i64; nnz];
    let mut data = vec![0.0f64; nnz];
    let pi_addr = indices.as_mut_ptr() as usize;
